...
rank1 = eval(open('rank.txt', encoding='utf-8').read())
rank_all = eval(open('rank_all.txt', encoding='utf-8').read())
def nation():
...
def find_data(id,num):
    ...
    c_x = country[1][:-2]
    output = eval(c_x)
    for i in range(len(output)): #11 14
        rank = rank_all[2018]
        if output[i][11] in rank.keys():
            output[i][11] = str(rank[output[i][11]])+'_'+output[i][11]
        else:
            output[i][11] = '220_弱鸡'
        if output[i][14] in rank.keys():
            output[i][14] = str(rank[output[i][14]]) + '_' + output[i][14]
        else:
            output[i][14] = '220_弱鸡'
        del output[i][15]
        del output[i][12]
...
def factual_data():
    ...
    for c in range(len(data)):
        i = str(c+1)
        url_x = data[i]['url_id']
        name = str(data[i]['rank'])+'_'+data[i]['nation']
        print(name)
