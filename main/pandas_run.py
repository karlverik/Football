...
class Football():
    ...
    def rank_go(self,name):
        file = pd.read_csv(PATH+'\\data\\nation_csv\\'+name+'.csv', encoding='utf-8')
        data = pd.DataFrame(file)
        #print(name)
        game_type = data['赛事英']
        home_team = data['主队']
        away_team = data['客队']
        team_score = data['全场比分']
        time = data['日期']
        win_rate = data['赛果盘路']
        #需要参数
        ...
        for i in count:
            ...
            if  time_int>= self.start_time and time_int<= self.end_time and '-' in str(team_score[i]):
...
def query(name):
    list = os.listdir(PATH+'\\main\\nation_csv')
    dict = {}
    for i in list:
        c = i.split('_')
        dict[c[1][:-4]] = c[0]
    print('[\''+str(dict[name])+'_'+name+'\']')
...
if __name__ == '__main__':
    pr = Football(name=ASA, game_type=False, time=[2010, 2018], rank=[1, 200])
    pr.mulprocess(8)
